...
def get_teams_by_contest_id(db: Session, contest_id: int, skip: int = 0, limit: int = 100) -> List[Team]:
    if hasattr(Team, "contest_id"):
        return (
            ...
            .filter(Team.contest_id == contest_id, Team.is_public == True)
            .offset(skip).limit(limit).all()
        )
    # contest_id 컬럼이 없으면 공개 팀만 반환(임시 동작)
    return get_public_teams(db, skip=skip, limit=limit)
